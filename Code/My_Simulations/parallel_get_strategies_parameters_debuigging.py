...
if __name__ == "__main__":
    ...
    discount_factor_list = [0.965]

    m_value = -6.5

    for discount_factor in discount_factor_list:

        for mode in ['ecological']:
            ...
            result_list_with_degraded_zero_and_flattened_list = [
                {
                    'final_point': tuple(results['final_point'][:, :,0].flatten()),
                    'avg_coop': float(results['avg_coop']),
                    'obsdist': results['obsdist']
                }
            ...
            original_dataframe.to_excel("debugging", index=False)

# %% 

# [[0.9993087649345398], [0.9994180202484131]]
