...
# AUTOGENERATED! DO NOT EDIT! File to edit: ../../nbs/01_EnvHistoryEmbedding.ipynb.
...
# %% ../../nbs/01_EnvHistoryEmbedding.ipynb 28
import numpy as np
...
# %% ../../nbs/01_EnvHistoryEmbedding.ipynb 32
...
# %% ../../nbs/01_EnvHistoryEmbedding.ipynb 47
...
# %% ../../nbs/01_EnvHistoryEmbedding.ipynb 55
...
# %% ../../nbs/01_EnvHistoryEmbedding.ipynb 65
...
# %% ../../nbs/01_EnvHistoryEmbedding.ipynb 70
...
# %% ../../nbs/01_EnvHistoryEmbedding.ipynb 75
...
# %% ../../nbs/01_EnvHistoryEmbedding.ipynb 83
def ObsActHistsIx(env, h):
    ...
    SAhists = StateActHistsIx(env, h=h)  #all possible state actions. Just remove the impossible trnasitions
    OAhists = _get_all_histories(env, h=h, attr='Q')  #all action histories given the states the states thatcan be ovseve
    ...
    # print(f"sahist[0]: {sahist[0]}, type: {type(sahist[0])}")
    # print(f"oahist[0]: {oahist[0]}, type: {type(oahist[0])}")

    # print(env.O)

    # print([env.O[:, sahist[k], oahist[k]] for k in range((env.N+1)*(hmax-h[0])+env.N, l, env.N+1)]) #it basically has to start form, th fforst atst. of tehre is is a . n stahte ecastns actions are , it 'll statr at tat 
    ...
    for oahist in OAhists:
        ...
        for sahist in SAhists:
            ...
            # print(sahist)
            sAs = [list(sahist[k:k+env.N]) for k in range(0, l, env.N+1)]   #split tuple into list of lists ofg thefgorm [(state, agent 1 action ... agent N action)] (state2, agent 1 action ... agent N action) ...]
            oAs = [list(oahist[k:k+env.N]) for k in range(0, l, env.N+1)]
            if sAs == oAs:
                # and then check whether oahist can be observed from sahist
                observable += np.prod([env.O[:, sahist[k], oahist[k]]  #for both agents pick out 
...
# %% ../../nbs/01_EnvHistoryEmbedding.ipynb 85
def hOset(env, h):
    hmax = max(h)
...
# %% ../../nbs/01_EnvHistoryEmbedding.ipynb 87
...
# %% ../../nbs/01_EnvHistoryEmbedding.ipynb 90
